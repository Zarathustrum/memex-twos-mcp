...
from load_to_sqlite import (  # noqa: E402
...
class TestIncrementalLoad:
    ...
    def test_fts_stays_in_sync(self, temp_db):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM things_fts WHERE things_fts MATCH 'dentist'"
        )
        dentist_count = cursor.fetchone()[0]

        cursor.execute(
            "SELECT COUNT(*) FROM things_fts WHERE things_fts MATCH 'doctor'"
        )
